...
<div className="fd-steps">
  ...
  <div className="fd-step">
    ...
    ```npm
    npm install better-auth@1.3.8 @better-auth/expo@1.3.8 --save-exact
  ...
  <div className="fd-step">
    ### Follow steps 5-7 of the Better Auth Expo guide

    The Expo integration for Better Auth requires a few involved steps that could change over time.
    Follow the last few steps of their Expo integration guide, beginning with
    [step 5](https://www.better-auth.com/docs/integrations/expo#initialize-better-auth-client).
...
### Better Auth with Expo Web

To use Expo Web, follow [step 4 of the React guide](https://convex-better-auth.netlify.app/framework-guides/react#set-environment-variables) to add your Expo Web site URL as the `SITE_URL` environment variable.

You will also need to add the site URL and the cross-domain plugin to the Better Auth instance:
...
<Callout>Note that the `expoClient` and `crossDomainClient` plugins cannot both be included in the client instance at the same time.</Callout>
