...
Schema-changing plugins that are not bundled with the component require
[Local Install](/local-install). This includes the
[`@better-auth/oauth-provider`](https://www.better-auth.com/docs/plugins/oauth-provider)
package used when your application acts as an OAuth or OIDC provider.
