...
### Querying auth tables from app code

When you read or write Better Auth tables from your app (outside the component),
use `authComponent.typedAdapter`. It wraps the adapter the same way as
`ctx.runQuery(components.betterAuth.adapter.findOne, …)` but infers document
types from `model`, and type-checks `where`, `select`, and `sortBy` against that
table.

**1. Pass your local schema to `createClient`** (from the install step above):
...
**2. Call `typedAdapter` from your queries and mutations:**
...
export const getUserByEmail = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Doc<"user"> | null — inferred from model
  },
});
```

`typedAdapter` exposes the same methods as the adapter API: `findOne`, `findMany`,
`create`, `updateOne`, `updateMany`, `deleteOne`, and `deleteMany`. See
[Type Utilities](/api/type-utilities#typedadapter-on-the-component-client).
